...
def similarity(city1, city2):
    set1 = set(normalize_cities(city1).split())
    set2 = set(normalize_cities(city2).split())
    
    return len(set1 & set2) / len(set1)

def city_mapping(df, metros):
    city_mapp = {}
    for city in df['city_full'].unique():
        best_match = None
        best_score = 0
        
        for metro in metros['metro_full'].unique():
            score = similarity(city, metro)
...
def clean_and_merge(df: pd.DataFrame, metros_path: str | None = "data/raw/usmetros.csv") -> pd.DataFrame:
    ...
    CITY_MAPPING = city_mapping(df, metros)
    # Normalize city_full
